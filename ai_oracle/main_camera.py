...
# Suppress noisy FFmpeg H.264 SEI warnings from the RTSP stream
...
from ai_oracle.dbClient import DBClient

app = typer.Typer(help="Ai_Oracle: Live Camera Stream", no_args_is_help=True)
console = Console()

@app.command()
def analyze(video_file: str = typer.Option(None, "--video-file", "-v", help="Path to local video file for testing")):
    ...
    camera_client = TpLinkCamera(video_file=video_file)
    ...
    # Launch GUI
    app_ui = VisionApp(camera_client=camera_client, motion_detector=motion_detector, yolo_vision=yolo_vision, ai_processor=ai_processor, db_client=db_client)
    app_ui.mainloop()
